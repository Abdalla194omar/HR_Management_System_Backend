  ...
  const lateEmployeesReply =
    language === "ar"
      ? `الموظفين المتجاوزين لساعات التأخير المسموحة لهذا الشهر هم: \n ${lateEmployees
          .map((emp) => `• ${emp.employeeData.firstName} ${emp.employeeData.lastName}`)
          .join("\n")}`
...
export async function attendanceReportFunc(language) {
  ...
  const [totalEmp, allAttendance, presentAttendance, absentAttendance, lateAttendance, overAttendance, lateSum, overSum] = await Promise.all([
    ...
    Attendance.countDocuments({ ...sharedQuery, status: "Present", lateDurationInHours: { $gt: 0 } }),
    Attendance.countDocuments({ ...sharedQuery, status: "Present", overtimeDurationInHours: { $gt: 0 } }),
...
⚠️ Lateness Rate: ${presentAttendance === 0 ? 0 : ((lateAttendance / presentAttendance) * 100).toFixed(2)}%\n
